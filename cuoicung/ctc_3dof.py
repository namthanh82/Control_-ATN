...
def ctc_3dof_components(
    ...
    for i in range(3):
        ...
        # ─── Friction ─────────────────────────────────────────────────────
        # Viscous (B·qdot): luôn luôn, mượt, tỷ lệ với tốc độ.
        ...
        # Convention ở đây: e = qd - q (line ~448). Vật lý: khi motor "kẹt" tại q ≠ qd,
        # cần thêm feedforward torque cùng chiều control để vượt ngưỡng ma sát tĩnh.
        # → stFricDir = sign(e) = sign(qd - q).
        #   q < qd (cần tăng q): torque control dương → thêm +T_s.
        #   q > qd (cần giảm q): torque control âm → thêm -T_s.
        ...
        if v_abs < model.stribeck_vel_thresh and model.static_friction[i] != 0.0:
            ramp = 1.0 - v_abs / model.stribeck_vel_thresh   # 1 tại qdot=0, 0 tại threshold
            stFricDir = _smooth_sign(e[i], k=80.0)           # sign(qd - q), hướng giảm |e|
