...
class TWAIController(threading.Thread):
    ...
    def _update_vel_estimates_locked(self, t_wall: float, v0_inst: float, v1_inst: float, v2_inst: float) -> None:
        """LP-filter vận tốc, sau đó tính acc bằng Savitzky-Golay trên vel đã lọc.

        Tính sẵn acc ở controller thread (giống ODESC) để GUI không phải gọi
        get_acc_jerk mỗi tick — giảm tải rất lớn cho GUI thread.
        """
        n_axes = 3
    ...
    def _parse_line(self, line: str):
        ...
        if line.startswith("FB,"):
            ...
            # Cấu trúc data tuple (gọn, 19 field — ODESC-style, đủ cho plot & log):
            ...
            #  16-18   : tau_out[0..2]     (Nm, torque gửi xuống ODrive, đã qua motor_sign)
