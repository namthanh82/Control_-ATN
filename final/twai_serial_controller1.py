...
class TWAIController(threading.Thread):
    ...
    def __init__(
        ...
        self.hip_link_com_distance_x1 = 0.0
        self.hip_link_com_distance_y1 = 0.0
        self.hip_link_com_distance_z1 = 1.91 * 10**(-3)
        self.hip_link_com_distance_x2 = 0.0 
        self.hip_link_com_distance_y2 = 0.0
        self.hip_link_com_distance_z2 = -11.52 * 10**(-3)
        self.knee_link_com_distance_x1 = 0.0
        self.knee_link_com_distance_y1 = 0.0
        self.knee_link_com_distance_z1 = 1.62 * 10**(-3)
        self.knee_link_com_distance_x2 = 0.0
        self.knee_link_com_distance_y2 = 0.0
        self.knee_link_com_distance_z2 = -12.74 * 10**(-3)
        self.hip_link_com_distance1 = sqrt(self.hip_link_com_distance_x1 ** 2 + self.hip_link_com_distance_y1 ** 2 + self.hip_link_com_distance_z1 ** 2)
        self.hip_link_com_distance2 = sqrt(self.hip_link_com_distance_x2 ** 2 + self.hip_link_com_distance_y2 ** 2 + self.hip_link_com_distance_z2 ** 2)
        self.knee_link_com_distance1 = sqrt(self.knee_link_com_distance_x1 ** 2 + self.knee_link_com_distance_y1 ** 2 + self.knee_link_com_distance_z1 ** 2)
        self.knee_link_com_distance2 = sqrt(self.knee_link_com_distance_x2 ** 2 + self.knee_link_com_distance_y2 ** 2 + self.knee_link_com_distance_z2 ** 2)
        self.hip_link_com_distance = (self.hip_link_mass1 * self.hip_link_com_distance1 + self.hip_link_mass2 * self.hip_link_com_distance2) / (self.hip_link_mass1 + self.hip_link_mass2)
        self.knee_link_com_distance = (self.knee_link_mass1 * self.knee_link_com_distance1 + self.knee_link_mass2 * self.knee_link_com_distance2) / (self.knee_link_mass1 + self.knee_link_mass2)
        self.hip_link_khau1 = (self.hip_link_mass1 * self.hip_link_com_distance1 + (self.hip_link_mass1 +self.hip_link_mass2) * self.hip_link_com_distance) / (2 * self.hip_link_mass1 + self.hip_link_mass2)
        self.hip_link_khau2 = (self.hip_link_mass2 * self.hip_link_com_distance2 + (self.hip_link_mass1 +self.hip_link_mass2) * self.hip_link_com_distance) / (self.hip_link_mass1 + 2 * self.hip_link_mass2)
        self.knee_link_khau1 = (self.knee_link_mass1 * self.knee_link_com_distance1 + (self.knee_link_mass1 +self.knee_link_mass2) * self.knee_link_com_distance) / (2 * self.knee_link_mass1 + self.knee_link_mass2)
        self.knee_link_khau2 = (self.knee_link_mass2 * self.knee_link_com_distance2 + (self.knee_link_mass1 +self.knee_link_mass2) * self.knee_link_com_distance) / (self.knee_link_mass1 + 2 * self.knee_link_mass2)
        self.ankle_link_com_distance = 57.23 * 10**(-3)
        self.hip_link_inertia1 = 0.0214189039
        self.hip_link_inertia2 = 0.026789179
        self.knee_link_inertia1 = 0.02099821589
        self.knee_link_inertia2 = 0.026421000
        self.hip_link_inertia =  self.hip_link_inertia1 + self.hip_link_inertia2 + self.hip_link_mass1 * self.hip_link_khau1 ** 2 + self.hip_link_mass2 * self.hip_link_khau2 ** 2  
        self.knee_link_inertia =  self.knee_link_inertia1 + self.knee_link_inertia2 + self.knee_link_mass1 * self.knee_link_khau1 ** 2 + self.knee_link_mass2 * self.knee_link_khau2 ** 2  
        self.hip_link_inertia_motor = self.big_motor_inertia_motor_inertia * gear_ratio_big ** 2
        self.knee_link_inertia_motor = self.ankle_inertia_motor = self.small_motor_inertia * gear_ratio_small ** 2 
        ...
        self.hip_mass2 = self.hip_link_mass2 = 3.277
        self.knee_mass1 = self.knee_link_mass1 = 2.898
        self.knee_mass2 = self.knee_link_mass2 = 3.201
        ...
        self.on_board_ctl        = True   # True: ESP32 chạy CTC + traj; PC chỉ gửi GOTO
    ...
    def _parse_line(self, line: str):
        ...
        if line.startswith("FB,"):
            ...
            # State from on-board CTC @ 50 Hz:
            #   S,q0,q1,q2, qdot0,qdot1,qdot2, tau0,tau1,tau2, motion_active
            try:
                parts = line[2:].split(",")
                if len(parts) >= 10:
                    with self.data_lock:
                        # Update feedback từ on-board (joint_deg đã trừ offset phía ESP32 chưa?
                        # → ESP32 báo joint_deg thô, host vẫn trừ offset như FB).
                        j0 = float(parts[0]); j1 = float(parts[1]); j2 = float(parts[2])
                        j0_off = j0 - self.joint_offset_deg[0]
                        j1_off = j1 - self.joint_offset_deg[1]
                        j2_off = j2 - self.joint_offset_deg[2]
                        if self.use_joint_feedback:
                            self.pos[0] = j0_off; self.pos[1] = j1_off; self.pos[2] = j2_off
                            self.joint_pos_deg[0] = j0_off
                            self.joint_pos_deg[1] = j1_off
                            self.joint_pos_deg[2] = j2_off
                        # Cập nhật vel/qdot từ on-board (đã được LP filter sẵn)
                        self.vel[0] = float(parts[3])
                        self.vel[1] = float(parts[4])
                        self.vel[2] = float(parts[5])
                        # Torque thực tế ESP32 đã gửi
                        self._tau_out[0] = float(parts[6])
                        self._tau_out[1] = float(parts[7])
                        self._tau_out[2] = float(parts[8])
                        self.torque_set[0] = self._tau_out[0]
                        self.torque_set[1] = self._tau_out[1]
                        self.torque_set[2] = self._tau_out[2]
                        # Cờ motion_active (1 = đang chạy GOTO)
                        was_armed = self.motion_armed
                        self.motion_armed = (int(parts[9]) == 1)
                        if was_armed and not self.motion_armed:
                            # Vừa kết thúc motion trên ESP32
                            self._motion_hold = True
                            self._motion_time_active = False
                            self.status_message = "Motion completed (on-board)"
                            self._setpoint_dirty = False
                        self.data.append((
                            time.time(),
                            self.pos[0], self.pos[1], self.pos[2],
                            self.pos_set[0], self.pos_set[1], self.pos_set[2],
                            self.acc_set[0], self.acc_set[1], self.acc_set[2],
                            self._tau_out[0], self._tau_out[1], self._tau_out[2],
                            self.torque_set[0], self.torque_set[1], self.torque_set[2],
                            self.pos[0], self.pos[1], self.pos[2],   # motor_deg (giả lập)
                            j0_off, j1_off, j2_off,
                        ))
