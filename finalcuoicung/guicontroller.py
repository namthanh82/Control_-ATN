...
class ControlGUI(tk.Tk):
    def __init__(self):
        ...
        self._default_ctrl = {
            ...
            "Kd1":            (8.0,   None),
            "Kd2":            (1.0,   None),
            "Control bandwidth": (1000.0, None),
            "Encoder bandwidth": (100.0,   None),
            "Max Trajectory Velocity": (60.0, "deg/s"),
        }
        self._ctrl_index = {}
        self.trajectory_var = tk.StringVar(value="Quintic")   # default giống ODESC
        ...
        self.plotting  = True
        self._last_t0  = None
        # ── Filter & error tracking (giống ODESC Control_GUI_Basic) ───────
        self.window_size  = 13      # SavGol window
        self.poly_order   = 2       # SavGol poly order
        self.max_jerk     = 0.0     # tracked trong _update loop
        self._max_jerk_var = tk.StringVar(value="0.000")
    ...
    def _build_ui(self):
        ...
        # ── Action buttons (ODESC-style: 3-col grid, status đầu, ESTOP cuối) ──
        ...
        # 1. Status Label (ODESC-style: 1 ô gọn)
        ...
        # 2. Offset (đổi tên từ "Set Offset" → "Offset" cho khớp ODESC)
        self.btn_offset = tk.Button(top_right, text="Offset",
        ...
        # Row 1: Close Loop / Stop Plot / Home
        ...
        self.btn_home.grid(row=1, column=2, sticky="nsew", padx=2, pady=2)

        # Row 2: Reset / ESTOP / PSO Kp/Kd
        ...
        self.btn_optimize_pso = ttk.Button(top_right, text="PSO Kp/Kd",
                                           command=self._on_optimize_pso, state="disabled")
        self.btn_optimize_pso.grid(row=2, column=2, sticky="nsew", padx=2, pady=2)
        ...
        # Trajectory type selector (giống ODESC Control_GUI_Basic — 5 loại)
        traj_frame = ttk.LabelFrame(ctrl_frame, text="Trajectory", padding=4)
        traj_frame.pack(pady=(4, 0), fill=tk.X)
        ttk.Label(traj_frame, text="Type:").pack(side=tk.LEFT, padx=(0, 6))
        self.trajectory_combo = ttk.Combobox(
            traj_frame,
            textvariable=self.trajectory_var,
            values=["Trapezoidal", "Cubic", "Quintic", "Spline", "Sinusoidal"],
            state="readonly",
            width=12,
        )
        ...
        # CTC mode selector (Scalar / Full M·C·G)
        self.scalar_ctc_var = tk.BooleanVar(value=False)
        ctc_mode_frame = ttk.LabelFrame(ctrl_frame, text="CTC Mode", padding=4)
        ctc_mode_frame.pack(pady=(4, 0), fill=tk.X)
        ttk.Checkbutton(
            ctc_mode_frame,
            text="Scalar (decoupled, like ODESC)",
            variable=self.scalar_ctc_var,
            command=self._apply_ctc_mode,
        ).pack(anchor=tk.W)
        ttk.Label(
            ctc_mode_frame,
            text="Off = full M(q)·C(q,qdot)·G(q) [more accurate if model correct]",
        ...
        # ── Filter settings (y hệt ODESC Control_GUI_Basic) ─────────────
        filter_frame = ttk.LabelFrame(self._right_frame, text="Filter", padding=8)
        ...
        self.poly_order_var = tk.StringVar(value=str(self.poly_order))
        ...
        self.btn_apply_filter = ttk.Button(filter_frame, text="Apply",
                                           command=self._on_apply_filter, state="disabled")
        self.btn_apply_filter.pack(pady=(10, 0), fill=tk.X)

        # ── Error display (y hệt ODESC Control_GUI_Basic) ────────────────
        error_frame = ttk.LabelFrame(self._right_frame, text="Error", padding=8)
        ...
        self.entry_max_jerk = ttk.Entry(error_frame, textvariable=self._max_jerk_var,
    ...
    def _on_offset(self):
        if self.ctrl:
            try:
                # Clear ODrive errors trước — nếu motor còn ERROR từ move trước,
                # set_offset không có tác dụng. Đặt cờ `clear_errors=True` để controller
                # tự gửi CLEAR qua ESP32 bridge.
                self.ctrl.set_offset(clear_errors=True)
                ...
                self.status_text.set("Status: offset đã set + ODrive errors cleared")
            except Exception:
    ...
    def _on_reset(self):
        if self.ctrl:
            try:
                # Reset cũng clear ODrive errors (firmware sẽ nhận CLEAR trước khi reset state).
                self.ctrl.reset(clear_errors=True)
                self.btn_estop.config(state="normal")
                self.btn_offset.configure(state="normal", bg="tomato")
                self.status_text.set("Status: đã reset + ODrive errors cleared")
            except Exception:
                logger.exception("Reset error")

    def _on_home(self):
        """Knee homing bằng limit switch EXT (GPIO 41). Firmware quay motor dir=+1
        (cùng chiều moment dương, duỗi thẳng) cho đến khi chạm switch, reset encoder
        count=0, gửi KOK về PC. Sau KOK mới có isOffset=True.

        Cần CLOSED_LOOP trước để position-hold tại EXT hoạt động (ODrive giữ chắc motor).
        Nếu chưa CLOSE, tự vào CLOSED_LOOP trước khi home.
        """
        if not self.ctrl:
            return
        try:
            # Đảm bảo CLOSED_LOOP để position-hold tại EXT có tác dụng
            state = self.ctrl.get_state()
            if state != CLOSED_LOOP_CONTROL and hasattr(self.ctrl, "enter_closed_loop"):
                self.ctrl.enter_closed_loop(clear_errors=True)
            ...
            self.status_text.set("Status: homing tới EXT(GPIO41)… bấm Set Offset sau khi xong")
        except Exception:
    ...
    def _on_mode_tog(self):
        ...
        try:
            ...
            if state != CLOSED_LOOP_CONTROL:
                ...
                # Controller sẽ tự CLEAR errors trước khi enter (an toàn + idempotent).
                self.ctrl.enter_closed_loop(clear_errors=True)
                self.btn_mode.config(text="CLOSE ✓", bg="yellow")
            else:
    ...
    def _sync_trajectory_mode(self):
        if not self.ctrl:
            return
        mode = (self.trajectory_var.get() or "Quintic").strip().lower()
        # Map ODESC-style names → internal mode của controller.
        # ODESC có 5 loại: Trapezoidal/Cubic/Quintic/Spline/Sinusoidal.
        # Controller hiện hỗ trợ: spline/quintic/cubic/trapezoidal (sinusoidal dùng 'spline' fallback).
        valid_modes = ("spline", "quintic", "cubic", "trapezoidal")
        if mode not in valid_modes:
            # Sinusoidal → fallback về spline (đủ mượt cho demo).
            mode = "spline"
    ...
    def _apply_ctc_mode(self):
        """Đổi giữa CTC đơn khớp (scalar) và full M·C·G."""
        if not self.ctrl:
            return
        use_scalar = bool(self.scalar_ctc_var.get())
        if hasattr(self.ctrl, "use_scalar_ctc"):
            self.ctrl.use_scalar_ctc = use_scalar
            mode = "SCALAR (decoupled, ODESC-style)" if use_scalar else "FULL (M(q)·C(q,qdot)·G(q))"
            self.status_text.set(f"Status: CTC mode = {mode}")
    ...
    def _on_move(self):
        ...
        try:
            ...
            # ODESC-style: restart motion clock mỗi lần user nhấn Run.
            # Controller giữ motion_armed=True giữa các lần nhấn → CTC chạy liên tục giữ pos.
            # Khi nhấn lại với target mới, _start_new_motion() reset _motion_t0 → spline chạy lại.
            if hasattr(self.ctrl, "_start_new_motion"):
                self.ctrl._start_new_motion()
            elif hasattr(self.ctrl, "motion_armed"):
    ...
    def _on_apply_filter(self):
        """Apply SavGol filter parameters (giống ODESC Control_GUI_Basic)."""
        try:
            ...
            # Apply lên controller + update local state
            self.window_size = window_size
            self.poly_order  = poly_order
            # Sync StringVar để Entry luôn hiển thị giá trị đã apply (kể cả khi user nhập "11.0" → "11").
            self.window_size_var.set(str(window_size))
            self.poly_order_var.set(str(poly_order))
            if hasattr(self.ctrl, "data_lock"):
                with self.ctrl.data_lock:
                    if hasattr(self.ctrl, "window_size"):
                        self.ctrl.window_size = window_size
                    if hasattr(self.ctrl, "poly_order"):
                        self.ctrl.poly_order  = poly_order
                    # Reset filter buffers để áp dụng kích thước mới.
                    # velFilBuf/timeFilBuf là LIST of 3 deques (1 cho mỗi axis),
                    # KHÔNG phải deque đơn — phải reset từng deque trong list,
                    # nếu không sẽ phá vỡ cấu trúc và gây IndexError ở _update_vel_estimates_locked.
                    from collections import deque as _dq
                    if hasattr(self.ctrl, "velFilBuf") and isinstance(self.ctrl.velFilBuf, list):
                        self.ctrl.velFilBuf = [_dq(maxlen=window_size) for _ in range(len(self.ctrl.velFilBuf))]
                    elif hasattr(self.ctrl, "velFilBuf"):
                        # Fallback: backend cũ dùng deque đơn (ODESC-style) — cũng reset.
                        self.ctrl.velFilBuf = _dq(maxlen=window_size)
                    if hasattr(self.ctrl, "timeFilBuf") and isinstance(self.ctrl.timeFilBuf, list):
                        self.ctrl.timeFilBuf = [_dq(maxlen=window_size) for _ in range(len(self.ctrl.timeFilBuf))]
                    elif hasattr(self.ctrl, "timeFilBuf"):
                        self.ctrl.timeFilBuf = _dq(maxlen=window_size)

            self.status_text.set(f"Status: filter applied (w={window_size}, p={poly_order})")
        except ValueError:
    ...
    def _update(self):
        try:
            ...
            if ctrl is None:
                ...
                self.btn_apply_filter.configure(state="normal" if connected and not estop else "disabled")
                ...
                if connected:
                    ...
                    # ── Update Error display (y hệt ODESC) ───────────────
                    # Lấy vel/vel_set để tính err.
                    with ctrl.data_lock:
                        v  = list(getattr(ctrl, "vel", [0.0, 0.0, 0.0]))
                        vs = list(getattr(ctrl, "vel_set", [0.0, 0.0, 0.0]))
                        acc_now = list(getattr(ctrl, "acc", [0.0, 0.0, 0.0]))
                    pos_err = max(abs(ps0 - p0), abs(ps1 - p1), abs(ps2 - p2))
                    vel_err = max(abs(vs[0] - v[0]), abs(vs[1] - v[1]), abs(vs[2] - v[2]))
                    self._set_entry(self.entry_pos_error, f"{pos_err:.3f}")
                    self._set_entry(self.entry_vel_error, f"{vel_err:.3f}")

                    # ── Track max jerk (numerical d(jerk)/dt từ acc diff) ──
                    if not hasattr(self, "_acc_prev"):
                        self._acc_prev = [0.0, 0.0, 0.0]
                        self._t_prev   = time.perf_counter()
                    t_now = time.perf_counter()
                    dt = t_now - self._t_prev
                    if dt > 0.001:
                        for axis in range(3):
                            j = (acc_now[axis] - self._acc_prev[axis]) / dt
                            if abs(j) > self.max_jerk:
                                self.max_jerk = abs(j)
                        self._acc_prev = acc_now[:]
                        self._t_prev   = t_now
                    self._max_jerk_var.set(f"{self.max_jerk:.3f}")
                ...
                if self.plotting and ctrl and connected:
                    ...
                    if data:
                        ...
                        # Data tuple MỚI (22 field, controller đã compute acc bằng SavGol):
                        #   d[0]   = timestamp
                        #   d[1..3]= pos[0..2],  d[4..6]= pos_set[0..2]
                        #   d[7..9]= vel[0..2],  d[10..12]= vel_set[0..2]
                        #   d[13..15]= acc[0..2] (deg/s^2, pre-computed)
                        #   d[16..18]= acc_set[0..2] (deg/s^2, ref acceleration từ spline)
                        #   d[19..21]= tau_out[0..2] (Nm, motor-side, đã qua motor_sign)
                        ...
                        # acc_set: ref acceleration từ spline (d[16..18]).
                        acc0_set = _arr[:, 16].tolist() if nd >= 19 else []
                        acc1_set = _arr[:, 17].tolist() if nd >= 20 else []
                        acc2_set = _arr[:, 18].tolist() if nd >= 21 else []
                        # Torque DK: d[19..21] (Nm motor-side, đã qua motor_sign).
                        tau0_vals = _arr[:, 19].tolist() if nd >= 22 else []
                        tau1_vals = _arr[:, 20].tolist() if nd >= 23 else []
                        tau2_vals = _arr[:, 21].tolist() if nd >= 24 else []
