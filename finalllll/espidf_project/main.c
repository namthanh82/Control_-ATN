...
static void send_torque_ramp_rate(uint8_t node_id, float rate) {
    // Set_Torque_Ramp_Rate doesn't exist as a CAN command (0x10) on ODrive 0.6.11.
    // Use Set_Property (cmd 0x05) with Endpoint ID 0x015 (torque_ramp_rate).
    // Endpoint encoding: 0x015 = (property_id << 5) | type, where type=1 (float).
    // For type=1 (float), property_id << 5 fits in 16 bits: 0x015 << 5 = 0x02A0.
    // The actual wire format for Set_Property:
    //   data[0..1]: Endpoint ID (LE) = (property_id << 5) | type
    //   data[2..7]: 6-byte payload (zero-padded for floats)
    uint8_t data[8] = {0};
...
// Auto-apply torque control mode + ramp rate at boot.
// ODrive resets control_mode to 3 (position) on every boot unless we set it.
// This avoids the user needing to send "CLOSE" before any torque command works.
static void auto_apply_torque_mode(void) {
    uart_write_str("INFO: Auto-applying torque mode (control_mode=1, input_mode=6, ramp=10 Nm/s)\n");
    for (int i = 0; i < 3; i++) {
        send_clear_errors(i);
...
// ============================================================================
//
// Protocol hỗ trợ 2 dạng:
//   1. Legacy text (backward compat):  TORQUE/POS/STATE/MODE/RAMP/CLEAR/CLOSE/IDLE/HOME/...
//   2. Short-form (PC driver dùng):    T{id}:val    P{id}:pos,vel
//                                       GOTO q0 q1 q2
//                                       HOLD / GAIN / VMAX / PRISM / CYL / LOCK / DX: / LP / S,
//
// Forward bridge mode: PC tính CTC và gửi torque/position xuống ESP32,
// ESP32 forward qua CAN tới ODrive. Tất cả CTC/trajectory chạy trên PC.
//
static void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        // Order matters: set INPUT_MODE first (sets controller mode via 0x0B),
        // then ramp rate via property write (0x05), then transition to CLOSED_LOOP.
        // Using Set_Property (cmd 0x05) because 0x10 (Set_Input_Torque_Ramp_Rate)
        // does not exist in ODrive firmware 0.6.11.
        for (int i = 0; i < 3; i++) {
            send_clear_errors(i);
            send_controller_mode(i, CONTROL_MODE_TORQUE_CONTROL, INPUT_MODE_TORQUE_RAMP);
            ...
            data[0] = (uint8_t)(endpoint & 0xFF);
            data[1] = (uint8_t)((endpoint >> 8) & 0xFF);
            write_float_le(&data[2], 10.0f);  // 10 Nm/s ramp
            send_can_message(make_can_id(i, ODRIVE_CMD_SET_PROPERTY), data, 8);
...
static void bridge_task(void* arg) {
    ...
    while (1) {
        ...
        //   FB,mot0_rev,mot1_rev,mot2_rev,j0_deg,j1_deg,j2_deg,tau0_Nm,tau1_Nm,tau2_Nm,motion
        // PC parses để lấy motor pos + joint pos + last torque sent.
        if (now_us - last_fb_us >= 10000) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                vel_deg_s[i] = (joint_deg[i] - last_joint_deg[i]) / 0.01f;
                last_joint_deg[i] = joint_deg[i];
            }

            uart_write_fmt("FB,%.6f,%.6f,%.6f,%.3f,%.3f,%.3f,%.4f,%.4f,%.4f,%d\n",
