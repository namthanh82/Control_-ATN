...
class ControlGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self._line_err2, = self.ax_vel.plot([], [], label="Error M2 (deg)", color="#4CAF50")
        ...
        self._line_acc2, = self.ax_acc.plot([], [], label="Accel M2", color="#4CAF50")
        ...
        # ── Error display ─────────────────────────────────────────────────
        err_frame = ttk.LabelFrame(self._right_frame, text="Sai số", padding=8)
        ...
        ttk.Label(err_frame, text="Error M2 (deg):").grid(row=2, column=0, sticky=tk.W)
        self.entry_err2 = ttk.Entry(err_frame, width=12, state="readonly")
        self.entry_err2.grid(row=2, column=1, padx=4, pady=2)
    ...
    def _show_idle_snapshot(self):
        ...
        # pos & setpoint đều ở world (pos + model_home, setpoint + model_home) để cùng hệ.
        try:
            mh = self.ctrl.get_model_home()
        except Exception:
            mh = (0.0, 0.0, 0.0)
        pos0_vals = [d[1] + mh[0] for d in data]
        pos1_vals = [d[2] + mh[1] for d in data]
        pos2_vals = [d[3] + mh[2] for d in data]
        set0_vals = [d[4] + mh[0] for d in data]
        set1_vals = [d[5] + mh[1] for d in data]
        set2_vals = [d[6] + mh[2] for d in data]  
        ...
        ax_pos.plot(t_rel, pos0_vals, label="Joint 0 (deg, world)", color="#2196F3")
        ax_pos.plot(t_rel, pos1_vals, label="Joint 1 (deg, world)", color="#FF9800")
        ax_pos.plot(t_rel, pos2_vals, label="Joint 2 (deg, world)", color="#4CAF50")
        ax_pos.plot(t_rel, set0_vals, label="J0 Setpoint (world)", color="#2196F3", linestyle="--", alpha=0.6)
        ax_pos.plot(t_rel, set1_vals, label="J1 Setpoint (world)", color="#FF9800", linestyle="--", alpha=0.6)
        ax_pos.plot(t_rel, set2_vals, label="J2 Setpoint (world)", color="#4CAF50", linestyle="--", alpha=0.6)
        ax_pos.set_ylabel("Position (deg, world frame)")
        ax_pos.grid(True)
    ...
    def _sync_trajectory_mode(self):
        ...
        self.ctrl.trajectory_mode = "quintic" if mode == "quintic" else "spline"
    ...
    def _ensure_error_entries(self):
        if not hasattr(self, "entry_err2"):
            self.entry_err2 = ttk.Entry(self, width=12, state="readonly")
        ...
        try:
            ...
            if ctrl is None:
                ...
                if connected:
                    ...
                    self._set_entry(self.entry_err2, f"{ps2 - p2:.3f}")
                ...
                if self.plotting and ctrl and connected:
                    ...
                    if data:
                        times     = [d[0] for d in data]
                        # pos & setpoint đều cộng model_home_deg để cùng hệ world → so sánh được.
                        # err = (set + mh) - (pos + mh) = set - pos (cùng hệ, đúng như cũ).
                        try:
                        ...
                        set1_vals = [d[5] + mh[1] for d in data]
                        set2_vals = [d[6] + mh[2] for d in data]
                        err0_vals = [d[4] - d[1] for d in data]   # err vẫn ở hệ tương đối (qd - q cùng hệ)
